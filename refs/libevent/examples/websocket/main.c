...
static void broadcast_msg(char *msg) {
  struct client *client;

  TAILQ_FOREACH(client, &clients, next) { evws_send_text(client->evws, msg); }
  log_d("%s\n", msg);
}

static void on_msg_cb(struct evws_connection *evws, int type,
...
static void on_close_cb(struct evws_connection *evws, void *arg) {
  client_t *client = arg;
...
static const char *nice_addr(const char *addr) {
...
static void addr2str(struct sockaddr *sa, char *addr, size_t len) {
  const char *nice;
  unsigned short port;
  size_t adlen;
...
static void on_html(struct evhttp_request *req, void *arg) {
  int fd = -1;
  struct evbuffer *evb;
  struct stat st;
...
int main(int argc, char **argv) {
  ...
  evhttp_bind_socket_with_handle(http_server, "0.0.0.0", 3030);
